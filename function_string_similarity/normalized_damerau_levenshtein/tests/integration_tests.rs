    ...
    fn test_append_over_http() {
        ...
        let query = r#"
            ...
            SELECT
                ?result
            WHERE {
                UNNEST(wf:call(strsim:normalizedDamerauLevenshtein, "stardog", "starlight") AS ?result)
            }
            "#;
        ...
        match response {
            Ok(r) => {
                ...
                assert_approx_eq!(bindings[0]["result"]["value"].as_str().unwrap().parse::<f64>().unwrap(), 0.5555555555555556)
            },
