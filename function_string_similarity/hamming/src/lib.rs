...
pub extern fn evaluate(subject: *mut c_char) -> *mut c_char {
    ...
    let string1 = binding["value_0"]["value"].as_str().unwrap();
    let string2 = binding["value_1"]["value"].as_str().unwrap();

    let result = hamming(string1, string2);

    let sparql_query_result = json!({
      "head": {"vars":["value_0"]}, "results":{"bindings":[{"value_0":{"type":"literal","value": result.ok()}}]}
    }).to_string();
