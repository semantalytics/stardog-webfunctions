...
mod tests {
    ...
    fn test_append_over_http() {
        ...
        let query = r#"
            prefix array: <http://wf.semantalytics.com/ipns/k51qzi5uqu5dlx0ttqevj64d3twk31y7hsgnofkqkjaiv11k98lj2rx60kjgv5/stardog/function/json/jmes/>
            prefix wf: <http://semantalytics.com/2021/03/ns/stardog/webfunction/latest/>

            SELECT
                ?result
            WHERE {
                BIND(wf:call(jmes:path, wf:call(array:of, "star"), "dog") AS ?result)
            }
            "#;
