...
mod tests {
    ...
    fn test_e_over_http() {
        ...
        let query = format!("prefix wf: <http://semantalytics.com/2021/03/ns/stardog/webfunction/{}/>", env::var("WEBFUNCTION_PLUGIN_VERSION").unwrap().as_str()) +
            r#"
            prefix constants: <http://wf.semantalytics.com/ipns/k51qzi5uqu5dlx0ttqevj64d3twk31y7hsgnofkqkjaiv11k98lj2rx60kjgv5/stardog/function/math/constants/>

            SELECT
                ?result
            WHERE {
                BIND(wf:call(constants:e) AS ?result)
            }
            "#;
        ...
        match response {
            Ok(r) => {
                ...
                assert_eq!(bindings[0]["result"]["value"], "2.718281828459045");
            },
