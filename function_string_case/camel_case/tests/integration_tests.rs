...
mod tests {
    ...
    fn test_append_over_http() {
        ...
        let query = r#"
            prefix str: <http://wf.semantalytics.com/ipns/k51qzi5uqu5dlx0ttqevj64d3twk31y7hsgnofkqkjaiv11k98lj2rx60kjgv5/stardog/function/string/>
            prefix wf: <http://semantalytics.com/2021/03/ns/stardog/webfunction/latest/>

            SELECT
                ?result
            WHERE {
                UNNEST(wf:call(str:camelCase, "stardog", "starlight") AS ?result)
            }
            "#;
