...
pub extern fn evaluate(subject: *mut c_char) -> *mut c_char {
    ...
    let array_literal_ids = array.trim_matches(|c| c == '[' || c == ']').split(',').trim().parse::<i64>().unwrap().collect();
    ...
    appended_array_literal_ids = array_literal_ids.append(id).iter().map(String::to_string).join(",").collect();
